...
function updateHtml() {
    ...
    content = content.replace(/transition-transform duration-500">/g, 'transition-transform duration-500 mix-blend-lighten">');

    // 2. Add volume options and convert currency
    const products = [
        { name: 'Tropical Mango', oldPrice: '29.99', newPrice: '149', emoji: '🥭', c1: 'orange', c2: 'yellow' },
        { name: 'Berry Blast', oldPrice: '29.99', newPrice: '149', emoji: '🍓', c1: 'pink', c2: 'red' },
        { name: 'Green Energy', oldPrice: '29.99', newPrice: '149', emoji: '🍏', c1: 'green', c2: 'emerald' },
        { name: 'Cosmic Blueberry', oldPrice: '31.99', newPrice: '159', emoji: '🫐', c1: 'blue', c2: 'indigo' },
        { name: 'Electric Grape', oldPrice: '31.99', newPrice: '159', emoji: '🍇', c1: 'purple', c2: 'pink' },
        { name: 'Watermelon Rush', oldPrice: '28.99', newPrice: '139', emoji: '🍉', c1: 'red', c2: 'rose' }
    ];

    for (const p of products) {
        // Regex to find the price and button section for this exact product
        const regexStr = `<div class="flex justify-between items-end mb-6">\\s*<div>\\s*<span class="text-xs text-gray-500 block">Price</span>\\s*<span class="text-2xl font-bold">\\$${p.oldPrice.replace('.', '\\.')}</span>\\s*</div>\\s*<div\\s*class="text-4xl opacity-20 group-hover:opacity-100 transition-opacity duration-300">\\s*${p.emoji}\\s*</div>\\s*</div>\\s*<button\\s*class="w-full py-3 rounded-xl bg-gradient-to-r from-${p.c1}-600 to-${p.c2}-500 font-bold text-white shadow-lg [^"]+"\\s*onclick="goToCheckout\\('${p.name}', ${p.oldPrice}\\)">`;
        const regex = new RegExp(regexStr);

        const replacement = `<div class="mb-4">
                                <span class="text-xs text-gray-500 block mb-2">Size</span>
                                <div class="flex space-x-2">
                                    <button type="button" class="px-3 py-1 text-xs border border-white/20 rounded hover:bg-white/10 transition-colors" onclick="updatePrice(this, ${parseInt(p.newPrice) - 60})">100ml</button>
                                    <button type="button" class="px-3 py-1 text-xs border border-white/20 rounded hover:bg-white/10 transition-colors bg-white/10" onclick="updatePrice(this, ${p.newPrice})">200ml</button>
                                    <button type="button" class="px-3 py-1 text-xs border border-white/20 rounded hover:bg-white/10 transition-colors" onclick="updatePrice(this, ${parseInt(p.newPrice) + 80})">350ml</button>
                                </div>
                            </div>
                            <div class="flex justify-between items-end mb-6">
                                <div>
                                    <span class="text-xs text-gray-500 block">Price</span>
                                    <span class="text-2xl font-bold price-display">₹${p.newPrice}</span>
                                </div>
                                <div class="text-4xl opacity-20 group-hover:opacity-100 transition-opacity duration-300">
                                    ${p.emoji}</div>
                            </div>
                            <button
                                class="w-full py-3 rounded-xl bg-gradient-to-r from-${p.c1}-600 to-${p.c2}-500 font-bold text-white shadow-lg hover:shadow-${p.c1}-500/50 transition-all transform hover:-translate-y-1 add-to-cart-btn"
                                data-product="${p.name}"
                                onclick="goToCheckout('${p.name}', '₹${p.newPrice}')">`;
...
`;
    ...
    content = content.replace('<!-- Contact Section -->', aboutHtml + '\\n' + reviewsHtml + '\\n    <!-- Contact Section -->');
...
`;
    content = content.replace('// Redirect to checkout page', jsCode + '\\n        // Redirect to checkout page');
